...
export const OrderHistorySpecView = ({ children, actions, style, onPress }) => {
    const { setLocations } = journeySlice.actions;
    const { toggleSideBar } = menuSlice.actions;
    const dispatch = useDispatch();


    const handleOnPress = (e) => {
        dispatch(setLocations({ index: 1, value: e.title }));
    ...
    return (
        <View style={{ ...styles.nav_container, ...style }}>
            <ScrollView
                horizontal
            ...
            >
                {(actions || []).map((e, idx) => {
                    return (
                        <TouchableOpacity key={idx} activeOpacity={0.7} onPress={() => handleOnPress(e)}>
