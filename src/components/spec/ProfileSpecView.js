...
import { PLATFORM_SHADOWS, PLATFORM_SHADOWS_DARK } from "../../data/settings";
import { useDispatch } from "react-redux";
import menuSlice from "../../redux/features/menu/menuSlice";


export const ProfileSpecView = ({ children, actions }) => {
    const { toggleSideBar } = menuSlice.actions;
    const dispatch = useDispatch();

    return (
        <View style={styles.nav_container}>
            <Text style={{ marginBottom: 20, fontWeight: "bold", fontSize: 15 }}>Profile.</Text>
            {(actions || []).map((e, idx) => {
                return (
